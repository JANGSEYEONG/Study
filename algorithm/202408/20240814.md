## 1. DP인 것 같으나, 반복문으로 처리

- 반복되는 최대 문자열 길이 찾기
- seen에 지금까지 확인된 문자 저장하며 하나라도 반복이 확인되면 다음 길이 체크하게 넘김
...
- 애니팡같은 문제. 3개 이상 세로가로 반복되면 없애고 미는 빡구현 문제
...
- queue에 넣을 때 특수 조건을 체크해야함. 딸기가 없으면 기사를 지나갈 수 없음.
  (딸기 없이 기사를 지나가는게 딸기를 들고 기사한테 다시 가는 최단경로일 수 있어서, 그러지 못하도록 반드시 체크해줘야함)
...
- 통나무 자르는 dp,, 작은 조각으로 최소 비용을 조합해서 사용
...
- 최소 회전 방향이라고 해서 무조건 직진 우선을 생각하지 말고, 일단 도달 가능한 최단거리를 구한 후 도착 지점 끝 조건으로 찾아서 최소 회전 수 구하기
